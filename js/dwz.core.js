...
(function($){
	...
	$.fn.extend({
		...
		layoutH: function($refBox){
			return this.each(function(){
				var $this = $(this);
				if($this.attr("fixedTable")){ // 此处添加固定列表格的布局变化
                    if (! $refBox) $refBox = $this.getLayoutBox();
                    var iRefH = $refBox.height();
                    var iLayoutH = parseInt($this.attr("layoutH"));
                    var iH = iRefH - iLayoutH > 50 ? iRefH - iLayoutH : 50;
                    if ($this.isTag("table")) {
                        $this.removeAttr("layoutH").wrap('<div layoutH="'+iLayoutH+'" style="height:'+iH+'px"></div>');
                    } else {
                    	if($this.attr("fixedTable")=="true"){
                            $this.height(iH);
						} else{
        ...
        isClass:function(tn) {
            if(!tn) return false;
            return $(this)[0].className.toLowerCase() == tn?true:false;
},
		/**
		 * 判断当前元素是否已经绑定某个事件
		 * @param {Object} type
