function initEnv() {
	...
	setTimeout(function() {
		...
		DWZ.regPlugins.push(function($p) {
			$("a[iconClass]", $p).each(function() {
				var imageName = $(this).attr("iconClass");
				var $span = $("span", this);
				if($span.length == 0) {
					// 用作<td>中的<a>     
					$(this).css({
						"background": "url(../images/button/" + imageName + ".png) no-repeat",
					...
					$span.css({
						"background-image": "url(../images/button/" + imageName + ".png)",
						"background-position": "0 3px"
					});
				}
			});
			if($.fn.pictureview) $(".ui-pictureview", $p).pictureview();
		});
...
function initUI($p) {
	...
	if($.fn.fixedTable) $("table.tablefixed", $p).fixedTable();
